...
instructions = arr.map(n => n.trim().split(/\s/))
...
for (var i = 0; i < instructions.length; i++) {
  condition = false
  inst = instructions[i]
  test = inst[inst.length - 2]
  reg = inst[inst.length - 3]
  val = Number(inst[inst.length -1])
  if (!output[reg]) output[reg] = 0
  if (!output[inst[0]]) output[inst[0]] = 0
  console.log(`inst: ${inst}`);
  switch (test) {
  case ">":
    console.log(`greater than ${reg}, ${val}`);
    if (output[reg] > val) {
      console.log("true");
      condition = true
    } else {
  ...
  if (condition == true) {
    console.log(`inst 1 is: ${inst[1]}`);
    switch (inst[1]) {
      case 'inc':
        console.log(`inc ${output[inst[0]]} with ${inst} by ${inst[2]}`);
        output[inst[0]] += Number(inst[2])
        console.log(output[inst[0]]);
        console.log(output);
        break;
      case 'dec':
      console.log(`dec ${output[inst[0]]} with ${inst} by ${inst[2]}`);
        output[inst[0]] -= Number(inst[2])
        console.log(output[inst[0]]);
        console.log(output);
        break;
    }
    if (output[inst[0]] > hightide) hightide = output[inst[0]]

  }
}

outarr = []
for (var reg in output) {
  if (output.hasOwnProperty(reg)) {
    console.log(`${reg} : ${output[reg]}`);
    outarr.push(output[reg])

  }
}

outarr.sort((a,b) => a-b)
console.log(`max is ${Math.max(...outarr)}, highest was ${hightide}`);
