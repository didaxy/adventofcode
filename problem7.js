...
console.log(`number of passphrases: ${rows.length}`);
//
// //split into 'columns'
let arr = rows.map(n => {
  let b = n.split(/\s+/)
...
let filtered = arr.filter(n => {return !containsDupe(n)})

function containsDupe(arr) {
//from problem 7 to problem 8
  // arr = arr.map(n => n.split('').sort().toString())
  let a, b
  for (var i = 0; i < arr.length; i++) {
    a = arr[i]
    for (var j = i + 1; j < arr.length; j++) {
      b = arr[j]
      // console.log(`i: ${i}, a: ${a}, j: ${j}, b: ${b}`);
      if (a == b) {
        console.log("match");
        return true
      }
...
console.log(`number filtered: ${filtered.length}`);
