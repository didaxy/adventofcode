...
for (var i = 0; i < input.length; i++) {
  ...
  input [i] = input[i].map(n => Number(n))
  input [i] = input[i].sort((a,b) => a - b)
  // console.log(input[i]);
  // let match = false;
  for (var j = 0; j < input[i].length; j++) {
    divisor = input[i][j]
    // console.log(`j is ${j}, divisor is ${divisor}`);
    for (var k = j + 1; k < input[i].length; k++) {
      let dividend = input[i][k]
      // console.log(`k is ${k}, dividend is ${dividend}`);
      if (dividend % divisor == 0) {
        //integer division
        console.log("integer division");
        output[i] = dividend / divisor
      }
    }
  }
  // let max = Math.max.apply(Math, input[i])
  // console.log(`max is ${max}`);
  // let min = Math.min.apply(Math, input[i])
...
//
let checksum = output.reduce((sum, n) => sum + n, 0)
//
