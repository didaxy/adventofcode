...
for (var i = 0; i < input.length; i++) {
  if(input[i].includes('->')) {
    arrstring = input[i].split('>').map(n => n.trim())
    outarr = arrstring[1].match(/[a-z]+/g)
    node = arrstring[0].split('(')
    nodename = node[0].trim()
    nodevalue = Number(node[1].split(')')[0])
    nodes.push({name: nodename, value: nodevalue, leaves: outarr})
    seen = seen.concat(outarr)
    // console.log(`seen ${seen}`);
  } else {
    leaf = input[i].split('(')
    leafname = leaf[0].trim()
    leafvalue = Number(leaf[1].split(')')[0])
    leaves.push({name: leafname, value: leafvalue})
  }
}
...
for (n of nodes) {
  ...
  for (var i = 0; i < n.leaves.length; i++) {
    nodeis = getLeaf(n.leaves[i]) || getNode(n.leaves[i])
    nodeweights[i] =  nodeis.total || nodeis.value
  }
  n.weights = nodeweights
  spread = Math.max(...nodeweights) - Math.min(...nodeweights)
  if (spread != 0) {
  spreadmin = spread
  console.log(`${spreadmin}  ${spread}`);
  console.log(`name: ${n.name}, totals: ${nodeweights}`);
}
}
...
function getValue(test) {
  ...
  // console.log(test);
  total += test.value
  if (!test || !test.leaves || test.leaves == "undefined") {
    // console.log(`this is a leaf`);
  } else {
    // console.log(`leaves: ${test.leaves}`);
    for (var i = 0; i < test.leaves.length; i++) {
      name = test.leaves[i]
      // console.log(name);
      leaf = getLeaf(name) || getNode(name)
      // console.log(leaf);
      total = total + getValue(leaf)
    }
  }
  return total;
}
...
console.log(get('apjxafk'));
// console.log(get('ugml'));


function get(item) {
  return getLeaf(item) || getNode(item)
}

// for (var i = 0; i < input2.length; i++) {
//   // console.log(`input: ${input[i]}`);
//
//     outarr = input2[i].split('(').map(n => n.trim())
//     thisone = outarr[0].trim()
//
//   // console.log(`thisone: ${thisone}`);
//   if (!seen.includes(thisone) > 0) {
//     console.log(`found ${thisone}`);
//   }
// }
