...
// toggle 2,2 through 8,8`

arr = input.trim().split('\n')
arr =arr.map(n => n.split(/\s/)).map(n => [n[n.length-4], makeCoords(n[n.length-3]), makeCoords(n[n.length-1])])
...
for (var i = 0; i < arr.length; i++) {
  ...
  iterator(f, arr[i][1], arr[i][2])
  // console.log(grid);
  // console.log(" ");
}

console.log(grid.reduce((sum,n) => sum + n.reduce((insum, m) => insum + m, 0), 0));

// console.log(grid);

function iterator(f, from, to) {
