...
//construct a like/dislike matrix
for (var i = 0; i < list.length; i++) {
  split = list[i].split(' ')
  if (!peeps[split[0]]) peeps[split[0]] = {}
  sign = split[2] == 'gain' ? 1 : -1
  peeps[split[0]][split[split.length - 1].slice(0, -1)] = split[3] * sign
}

console.log(peeps);

//generate all possible configuarations
let peeparr = []

for (var perp in peeps) {
  if (peeps.hasOwnProperty(perp)) {
    peeparr.push(perp)
  }
}
...
let combos = permutator(peeparr)
let scores = []

scores = combos.map(n => score(n))
...
function score(combo) {
  // console.log(`just scoring this combo: ${combo}`);
  let sum = 0, li, ri, left, right, perp, incr
  for (var i = 0; i < combo.length; i++) {
    let perp = combo[i]
    li = i - 1
    ri = i + 1
    if (li == -1) li = combo.length - 1
    if (ri == combo.length) ri = 0
    left = combo[li]
    right = combo[ri]
    // console.log(`looking at ${perp} who has ${left} to the left and ${right} to the right`);
    incr = (peeps[perp][left] || 0) + (peeps[perp][right] || 0)
    // console.log(`${perp}s contribution is ${incr}`);
    sum = sum + incr
